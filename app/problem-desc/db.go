...
func (db *DBImpl) Find(problemId uint, page, pageSize int) (objs []problem_desc.ProblemDesc, err error) {
	return objs, db.GORMDBImpl.Page(page, pageSize).Where("pid = ?", problemId).
		Find(&objs).Error
}

func (db *DBImpl) Count(problemId uint) (cnt int64, err error) {
	return cnt, db.GORMDBImpl.DB.Model(&db.idleObject).Where("pid = ?", problemId).
		Count(&cnt).Error
}
